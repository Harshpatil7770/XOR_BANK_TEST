...
public class ManagerServiceImplDetails {
	...
	public Manager addNewManager(ManagerDTO managerDTO) {
		...
		try {
			...
			log.error("ManagerServiceImplDetails - Exception occured while adding new manager details ", e);
	...
	public Manager updateManagerDetails(ManagerDTO managerDTO) {
		try {
			...
			Address updateExistingAddress = addressRepo.findById(managerDTO.getAddressDTO().getAddressId())
					.orElse(null);
